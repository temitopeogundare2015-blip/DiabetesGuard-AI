...
Fixes      : dark mode text · categorical risk scoring · emoji removed ·
             increased agent token limits · syntax clean
"""
...
UCI Diabetes 130-US Hospitals (101,766 encounters, 42 features, 1999-2008):
  ...
  - Demo-only AUC=0.529 to Demo+Utilisation AUC=0.629  (H2 SUPPORTED, +19%)
...
UCI low F1 reflects severe class imbalance (only 11% readmitted within 30 days).
"""

def ask_agent(system_prompt, user_prompt, max_tokens=600):
    try:
...
def train_models():
    ...
    upre = ColumnTransformer([
        ...
        ("c", Pipeline([("i", SimpleImputer(strategy="most_frequent")),
                        ("o", OneHotEncoder(handle_unknown="ignore", sparse_output=False))]), ucc),
    ])
    ulr = Pipeline([("p", upre), ("m", LogisticRegression(max_iter=1000, C=1.0, solver="lbfgs", random_state=42))])
    urf = Pipeline([("p", upre), ("m", RandomForestClassifier(n_estimators=100, max_depth=10, random_state=42, n_jobs=-1))])
    ulr.fit(Xtr, Ytr)
    urf.fit(Xtr, Ytr)
    ...
    def fauc(cols):
        ...
        p = ColumnTransformer([
            ("n", SimpleImputer(strategy="median"), nc),
            ("c", Pipeline([("i", SimpleImputer(strategy="most_frequent")),
                            ("o", OneHotEncoder(handle_unknown="ignore", sparse_output=False))]), cc)
        ])
    ...
    uci_m = {
        "LR": {"AUC": round(roc_auc_score(Yte, ulrp), 3), "F1": round(f1_score(Yte, ulrpred), 3),
               "Rec": round(recall_score(Yte, ulrpred), 3), "Prec": round(precision_score(Yte, ulrpred, zero_division=0), 3),
               "prob": ulrp, "pred": ulrpred, "y": Yte.values},
        "RF": {"AUC": round(roc_auc_score(Yte, urfp), 3), "F1": round(f1_score(Yte, urfpred), 3),
               "Rec": round(recall_score(Yte, urfpred), 3), "Prec": round(precision_score(Yte, urfpred, zero_division=0), 3),
               "prob": urfp, "pred": urfpred},
    }

    # Secondary
    ss = sec.copy()
    ss["y"] = (ss["readmitted"] == "yes").astype(int)
    Xs = ss.drop(columns=["readmitted", "y"])
    Ys = ss["y"]
    ...
    slr = Pipeline([("p", spre), ("m", LogisticRegression(max_iter=1000, C=1.0, solver="lbfgs", random_state=0))])
    srf = Pipeline([("p", spre), ("m", RandomForestClassifier(n_estimators=100, max_depth=10, random_state=0, n_jobs=-1))])
    slr.fit(Xstr, Ystr)
    srf.fit(Xstr, Ystr)
    ...
    sec_m = {
        "LR": {"AUC": round(roc_auc_score(Yste, slrp), 3), "F1": round(f1_score(Yste, slrpred), 3),
               "Rec": round(recall_score(Yste, slrpred), 3), "Prec": round(precision_score(Yste, slrpred, zero_division=0), 3),
               "prob": slrp, "pred": slrpred, "y": Yste.values},
        "RF": {"AUC": round(roc_auc_score(Yste, srfp), 3), "F1": round(f1_score(Yste, srfpred), 3),
               "Rec": round(recall_score(Yste, srfpred), 3), "Prec": round(precision_score(Yste, srfpred, zero_division=0), 3),
               "prob": srfp, "pred": srfpred},
    }
    return uci_m, sec_m, auc_demo, auc_util, uci, sec

# ── Risk scoring — categorical-aware ─────────────────────────────────────────
def compute_risk(inpatient, los, emergency, outpatient, diagnoses, medications,
                 hba1c, age="[50-60)", race="Caucasian", insulin="No", gender="Female"):
    score = (inpatient * 0.35 + los * 0.25 + emergency * 0.20 +
             outpatient * 0.10 + diagnoses * 0.07 + medications * 0.03)

    age_mult = {
        "[0-10)": 0.70, "[10-20)": 0.75, "[20-30)": 0.80, "[30-40)": 0.85,
        "[40-50)": 0.90, "[50-60)": 1.00, "[60-70)": 1.10,
        "[70-80)": 1.20, "[80-90)": 1.28, "[90-100)": 1.35,
    }
    score *= age_mult.get(age, 1.0)
    ...
    if   pct >= 60: return pct, "High",     "#e74c3c", "High"
    elif pct >= 35: return pct, "Moderate", "#f39c12", "Moderate"
    else:           return pct, "Low",      "#27ae60", "Low"
...
.agent-box p, .agent-box li, .agent-box strong, .agent-box ul, .agent-box ol {
    color: #1a1a2e !important;
}
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if submitted:
        risk_pct, level, colour, _ = compute_risk(
            inpatient, los, emergency, outpatient, diagnoses, medications, hba1c,
            age=age,
            race=race if using_uci else "Caucasian",
            insulin=insulin,
            gender=gender if using_uci else "Female",
        )
        patient = {
        ...
        with g_col:
            ...
            ax.text(0, -0.55, f"{level} Risk", ha="center", fontsize=11,
                    color="black", fontweight="bold")
            ax.axis("off")
            st.pyplot(fig, use_container_width=True)
            plt.close()
            st.progress(risk_pct / 100)
            st.caption(f"Scored using top-weighted features from your "
        ...
        with a_col:
            ...
            with st.spinner("Assessing readmission risk..."):
                a1_out = ask_agent(
                    ...
                    "State risk level clearly, reference the most concerning utilisation features, "
                    "and note which dataset context is being used and what that means for AUC reliability.",
        ...
        with st.spinner("Explaining risk drivers..."):
            a2_out = ask_agent(
                ...
                "Be specific about the actual feature values.",
        ...
        with st.spinner("Generating discharge plan..."):
            a3_out = ask_agent(
                ...
                "Give exactly 4 specific, empathetic, and fully detailed discharge recommendations "
                "to reduce this patient's 30-day readmission risk. For each recommendation include: "
                "an Actionable Step, why it matters clinically, and an empathetic message to the patient. "
                "Tailor everything to their age group, HbA1c level, prior visit history, and insulin status. "
                "Use plain, patient-friendly language. Do not cut off — complete all 4 recommendations fully.",
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with tab2:
        ...
        st.success(f"H2 Supported: AUC {ad} to {au} (+{round(au - ad, 3)}) when utilisation features added.")
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    cc = ["#e74c3c", "#e67e22", "#f39c12", "#3498db", "#2ecc71", "#9b59b6", "#95a5a6"]
    ca, cb = st.columns(2)
    with ca:
        ...
        bars = ax4.barh(feats[::-1], imps[::-1], color=cc[::-1])
    ...
    with cb:
        ...
        bars5 = ax5.barh(sfeats[::-1], simps[::-1], color=cc[::-1])
    ...
    with c1:
        ...
        ax6.pie([53.9, 34.9, 11.2],
                labels=["Not Readmitted (NO)", "Readmitted >30d", "<30d (Target)"],
    ...
    st.info("UCI is heavily imbalanced (11% readmitted <30d) — explaining the low F1. "
